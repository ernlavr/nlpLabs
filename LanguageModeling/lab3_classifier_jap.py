...
from transformers import AutoTokenizer, AutoModel, AdamW, AutoModelForCausalLM, T5Tokenizer
...
dataset = dataset.filter( lambda df: df["language"].startswith("japanese") )
...
model_name = 'best_jap_model_GPT2'
...
def collate_fn( input ):
    ...
    for inp in input:
        ...
        if 'minimal_answers_start_byte' in context:
            ans_start = context['minimal_answers_start_byte'][0]
            ans_ends = context['minimal_answers_end_byte'][0]

        if 'answer_start' in context:
            ans_start = context['answer_start'][0]
            ans_ends = ans_start + len( context['answer_text'][0] )

        if 'answer_start' in context:
            answerable = context['answer_start'][0] != -1
        elif 'minimal_answers_start_byte' in context:
            answerable = context['minimal_answers_start_byte'][0] != -1

        input = [ f'<question>{question}<context>{answer}' ]

        num_of_tokens = len(tokenizer.tokenize( input[0] ))
        num_of_tokens_in_answer = len(tokenizer.tokenize( answer[ans_start:ans_ends] ))

        if num_of_tokens >= 1024:

            offset = ( 1024 - num_of_tokens_in_answer ) // 2
            new_start = 0 if ans_start - offset < 0 else ans_start - offset
            input[-1] = f'<question>{question}<context>{answer[new_start:ans_ends+offset]}'
...
print( f"f1_score={ f1_score( yval, pred )} accuracy={ accuracy_score( yval, pred )}" )
