...
dataset = dataset.filter( lambda df: df["language"].startswith("english") ) #or df["language"].startswith("finnish") or df["language"].startswith("japanese") )
...
tokenizer = GPT2Tokenizer.from_pretrained( 'distilgpt2', do_lower_case=True)
